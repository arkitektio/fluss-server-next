...
Ported from mikro's ``core/scoping.py``. Note the fluss data model: ``Run`` /
``Snapshot`` / ``RunEvent`` reach an organization only through a *nullable*
``flow`` FK, which :func:`organization_path` (non-nullable paths only) cannot
follow — those are scoped with explicit ``flow__organization`` /
``run__flow__organization`` filters at the call sites rather than via
:func:`for_org`. ``ReactiveTemplate`` is a global, org-less catalog and is
registered in :data:`UNSCOPED_MODELS`.
"""
...
# ReactiveTemplate is a shared catalog with no organization at all. Run /
# Snapshot / RunEvent / Trace* reach an organization only via a nullable `flow`
# FK; they are scoped through explicit relation filters at the call sites, so
# for_org() is intentionally not used for them.
UNSCOPED_MODELS: frozenset[str] = frozenset({"ReactiveTemplate"})
    ...
    if path is None:
        if model.__name__ not in UNSCOPED_MODELS:
            raise LookupError(
                ...
                "registered in reaktion.scoping.UNSCOPED_MODELS"
            )
