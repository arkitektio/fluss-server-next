# Fluss — Configuration Reference

This document explains how the **fluss** service is configured, then lists every
...
[`fluss_server/configuration.py`](fluss_server/configuration.py); this file documents it
for humans. If the two ever disagree, the code wins — and you can always print the live,
...
```bash
ARKITEKT_CONFIG_FILE=/etc/fluss/config.yaml python manage.py runserver
```
...
| `django.admin.password` | `DJANGO__ADMIN__PASSWORD` |
